...
public:
	...
	friend String operator+( String s, const String &s2 ) { s += s2; return s; } 
	...
	{
		size_t index = str.find_first_of( find.str, 0 );
		while( index != std::string::npos ) {
			str.replace( index, find.str.length(), replace.str );
			index = str.find_first_of( find.str, index + replace.str.length() );
		}
	}
